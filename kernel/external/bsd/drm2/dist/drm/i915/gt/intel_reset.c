...
static int gen8_engine_reset_prepare(struct intel_engine_cs *engine)
{
	...
	ret = __intel_wait_for_register_fw(uncore, reg, mask, ack,
					   700, 0, NULL);
	if (ret) {
		u32 _mi = intel_uncore_read_fw(uncore,
					       RING_MI_MODE(engine->mmio_base));
		DRM_ERROR("%s reset req timed out: RESET_CTL=%08x MI_MODE=%08x idle=%d\n",
			  engine->name, intel_uncore_read_fw(uncore, reg),
			  _mi, !!(_mi & MODE_IDLE));
		/*
		 * LISPBSD: the ready-to-reset handshake works fine once the
		 * engine's command streamer is idle (measured ~5us), but the
		 * early boot sanitize can catch it before it has stopped.
		 * Explicitly stop the CS, wait for idle, and retry once.
		 */
		intel_uncore_write_fw(uncore,
				      RING_MI_MODE(engine->mmio_base),
				      _MASKED_BIT_ENABLE(STOP_RING));
		(void)__intel_wait_for_register_fw(uncore,
				      RING_MI_MODE(engine->mmio_base),
				      MODE_IDLE, MODE_IDLE, 1000, 0, NULL);
		intel_uncore_write_fw(uncore, reg, _MASKED_BIT_ENABLE(request));
		ret = __intel_wait_for_register_fw(uncore, reg, mask, ack,
						   2000, 0, NULL);
		DRM_ERROR("%s after stop_cs+retry: RESET_CTL=%08x ret=%d\n",
			  engine->name, intel_uncore_read_fw(uncore, reg), ret);
	}
