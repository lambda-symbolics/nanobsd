...
/*
 * lpsched - LISPBSD laptop power scheduler: state and sysctl tree.
 *
 ...
 *	machdep.lpsched.estcpu_thresh	interactive threshold (estcpu>>11)
...
int	lpsched_fgpid = 0;
int	lpsched_estcpu_thresh = 2;
volatile int		lpsched_last_input;
volatile uint8_t	lpsched_idle_depth[MAXCPUS];
...
SYSCTL_SETUP(sysctl_lpsched_setup, "sysctl machdep.lpsched subtree setup")
{
	...
	sysctl_createv(clog, 0, &node, NULL,
	    ...
	    SYSCTL_DESCR("LWPs with estcpu>>11 below this count as interactive"),
